...
def drew_loss_accuracy(loss, accuracy, epochs, lr):
    ...
    ax1.set_title('Tanh, learning rate = {}'.format(lr))
    ax1.set_xlabel('epoch')
    ax1.set_ylabel('loss')
    ax2.plot(x, accuracy, 'm.-.', label='accuracy', linewidth=1)
    ax2.set_title('Tanh, learning rate = {}'.format(lr))
    ax2.set_xlabel('epoch')
    ax2.set_ylabel('accuracy(%)')
    plt.savefig('./picture/Tanh_{}.png'.format(lr))


def draw_accuracy(accuracy, learning_rates):
    fig = plt.figure(figsize=(6, 12))
    ax1 = fig.add_subplot(1, 1, 1)
    ax1.set_title('Accuracy Vs Learning Rate(Relu)')
    ax1.set_xlabel('Learning_rate',)
    ax1.set_ylabel('Accuracy(%)')
    ax1.plot(learning_rates, accuracy, 'm.-.', label='loss', linewidth=1)
    plt.savefig('./picture/Accuracy_Vs_LR.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    for lr in lr_list:
        ...
        # -------------保存------------
        testing(testing_s, Model, accuracyInLr)
        #torch.save(Model.state_dict(), './model/sigmoid_{}.ckpt'.format(lr))
    draw_accuracy(accuracyInLr, lr_list)