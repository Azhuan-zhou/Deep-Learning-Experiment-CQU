...
class MyLSTM(nn.Module):
    ...
    def forward(self, text):
        ...
        h_0, c_0 = self.init_hidden_state(embedded.size(1))
        _, (h_n, c_n) = self.lstm(embedded, (h_0, c_0))
        out = self.fc(h_n)
        out = self.relu(out)
        out = self.fc2(out)
        return out[0, :, 0]
    ...
    def init_hidden_state(self, batch_size):
        h_0 = torch.rand(1, batch_size, self.hidden).to(torch.device('cpu'))
        c_0 = torch.rand(1, batch_size, self.hidden).to(torch.device('cpu'))
        return h_0, c_0
...
if __name__ == '__main__':
    ...
    hiddden = 256
    output = 1
    model = MyLSTM(input, embedding, hiddden, output).to(device)
